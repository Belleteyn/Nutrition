...
FoodPtr createPortion(uint16_t& portion, const FoodAvailable& avFood, const FoodAvailable::Daily& daily
             , const Nutrition& ideal, const float allowedError)
{
  FoodPtr food = FoodPtr(new Food(avFood.food));
  ...
  {
    std::cout << "overheading: " << overheading << " for " << food->getName() << ", portion " << food->getPortionMass() << std::endl;
    return nullptr;
  }
...
FoodTree::SubTree createSubTree(const FoodAvailable& avFood, const Nutrition& ideal, const float allowedError)
{
  FoodTree::SubTree subTree;
  ...
  auto food = createPortion(portion, avFood, daily, ideal, allowedError);
  if (!food)
  {
    return subTree;
  }

  subTree.emplace_back(FoodNode(food));
...
static std::multimap<int16_t, FoodAvailable> giMap = {
  ...
  GIPair(60, FoodAvailable(Food("banana", 1.5, 21.8, 0.2, 95), 120 * 8, 120, FoodAvailable::Daily(120 * 8, 120))),
  ...
  GIPair(55, FoodAvailable(Food("oatmeal mistral", 13.8, 69.8, 5.9, 355), 400, 40, FoodAvailable::Daily(80, 40, 30))),
  ...
  GIPair(0, FoodAvailable(Food("red salmon", 19, 0, 10, 135), 60, 10, FoodAvailable::Daily(200, 100))),
  GIPair(0, FoodAvailable(Food("kefir 1%", 2.8, 4, 1, 36), 450, 220, FoodAvailable::Daily(440, 220))),
...
int main()
{
  ...
  for (auto iter = giMap.begin(); iter != giMap.end(); ++iter)
  {
    ...
    {
      N *= sub.size();
      tree.addSubTree(sub, overheadingComparator);
      //tree.print();
    }
  }
